...
class ReaderHelperApp:
    ...
    def draw_dialog(canvas, dlg_summary):
        ...
        dw, dh = 900, 500
        # POSITION CHANGED: Top-Right Corner (with 30px padding)
        dx0 = SCREEN_W - dw - 30  
        dy0 = 30                  
        
        cv2.rectangle(canvas, (dx0,dy0),    (dx0+dw,dy0+dh), (25,25,35),    -1)
        # COLOR CHANGED: Red border
        cv2.rectangle(canvas, (dx0,dy0),    (dx0+dw,dy0+dh), (0,0,255),  2) 
        cv2.rectangle(canvas, (dx0,dy0),    (dx0+dw,dy0+52), (40,40,60),    -1)
        # COLOR CHANGED: Red text and line
        cv2.putText(canvas, "Looks like you might be stuck on this paragraph.",
                    (dx0+20, dy0+36), cv2.FONT_HERSHEY_DUPLEX, 0.85, (0,0,255), 1, cv2.LINE_AA)
        cv2.line(canvas, (dx0, dy0+52), (dx0+dw, dy0+52), (0,0,255), 1) 
